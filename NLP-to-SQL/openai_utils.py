...
def get_openai_key():
    """
    This function reads openai api key from a file called openai-api-key.txt
    """
...
def create_table_definition_prompt(df, table_name):
    ...
    prompt = f'''### sqlite table, with its properties:
#
# {table_name}({",".join(str(x) for x in df.columns)})
#
'''
    return prompt
